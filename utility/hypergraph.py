...
import numpy as np
from utility.base import BaseHypergraph
...
class Hypergraph(BaseHypergraph):
    ...
    def __init__(
            ...
            merge_op: str = "mean",
            ui_graph: Optional[torch.Tensor] = None,
            iu_graph: Optional[torch.Tensor] = None,
    ...
    def add_hyperedges(self, e_list: Union[List[int], List[List[int]]],
            e_weight: Optional[Union[float, List[float]]] = None,
            merge_op: str = "mean",
            group_name: str = "main",
    ):
        ...
        for _idx in range(len(e_list)):
            test_index = (_idx,)
            self._add_hyperedge(
                self._hyperedge_code(e_list[_idx], e_list[_idx], test_index), {"w_e": float(e_weight[_idx])}, merge_op, group_name,
            )
    ...
    def H(self) -> torch.Tensor:
        ...
        if self.cache.get("H") is None:
            ...
            self.cache["H"] = self.iu_graph.to(self.device)
        return self.cache["H"]
    ...
    def H_T(self) -> torch.Tensor:
        ...
        if self.cache.get("H_T") is None:
            ...
            self.cache["H_T"] = self.ui_graph.to(self.device)
    ...
    def v2e_aggregation(
            self, X: torch.Tensor, aggr: str = "mean", v2e_weight: Optional[torch.Tensor] = None, drop_rate: float = 0.08
    ):
    ...
    def v2v(
        ...
        X_e = self.v2e(X, v2e_aggr, v2e_weight, e_weight, drop_rate=v2e_drop_rate)
        X_n = self.e2v(X_e, e2v_aggr, e2v_weight, drop_rate=e2v_drop_rate)
        # numpy_array = X_n.numpy()
