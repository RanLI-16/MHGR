...
class HGNNPConv(nn.Module):
    ...
    def forward(self, X: torch.Tensor, hg: Hypergraph, v2e_weight: torch.Tensor, e2v_weight: torch.Tensor) -> tuple[Any, Any]:
        ...
        """
        X = self.theta(X)
        X_n, X_e = hg.v2v(X, aggr="softmax_then_sum", v2e_weight=v2e_weight, e2v_weight=e2v_weight)  # softmax_then_sum
        if self.bn is not None:
            X_e = self.bn(X_e)
            X_n = self.bn(X_n)
        X_n = self.drop(self.act(X_n))
        X_e = self.drop(self.act(X_e))

        # if not self.is_last:
        #     X, _ = hg.v2v(X, aggr="softmax_then_sum", v2e_weight=v2e_weight, e2v_weight=e2v_weight)  # softmax_then_sum
        #     if self.bn is not None:
        #         X = self.bn(X)
        #     X = self.drop(self.act(X))
        #
        # if self.is_last:
        #     X_n, X_e = hg.v2v(X, aggr="softmax_then_sum", v2e_weight=v2e_weight, e2v_weight=e2v_weight)
        #     return X_n, X_e
